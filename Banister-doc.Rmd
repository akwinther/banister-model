...
###Creating models
Now that we have opimized each athletes parmeters, and organised the metrics into neatly organised lists, we can use the magic that is lapply to create six athlete specific models. 
```{r}
...
###Visualization
Now that we have fitted our models we can start thinking about ways to visualize them. A great package to do this is ggplot, however ggplot only takes data frames as inputs therefore we first have to transform our lists. We're also going to create a new variable called weeks so that we can visualize the weekly progression in performance, rather than day by day. To do this we're going to use the dplyr package: 
```{r warning = FALSE}
