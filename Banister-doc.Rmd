---
title: "Fitting individualized Banister models in R"
...
Now that we've neatly organised our data, it's time to write our banister function.

###Writing the Banister function
If we look at the R Documentation for the *optim* function (which can be done by typing ?optim in the console), we can see that the first argument of our banister function needs to contain a vector with the parameters we want to optimize for. In our case this will be the five adjustible parameters associated with the banister model: the initial performance level $p(0)$, the two time constants ($k_1$, $k_2$), and the two gain parameters ($\tau_1$, $\tau_2$). Rememeber, we're trying to predict performance and therefore want to minimize the error between our model and the actual performance. To do this we'll use the sum of squared errors as the metric to be minimised. 
```{r}
...
###Creating models
Now that we have opimized each athletes parmeters, and organised the metrics into neatly organised lists, we can use the magic that is *lapply* to create six subject-specific models. 
```{r}
...
###Visualization
Now that we have fitted our models we can start thinking about ways to visualize them. A great package to do this is *ggplot*, however *ggplot* only takes data frames as inputs therefore we first have to transform our lists. We're also going to create a new variable called *'Week'* so that we can visualize the weekly progression in performance, rather than day by day. To do this we're going to use the *dplyr* package: 
```{r warning = FALSE}
...
#Final notes
And there you have it. As showed, the banister model nicely accomodates the non-linear nature of the data for each subject. However, 
